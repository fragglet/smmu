...
void HU_LoadFont()
{
        ...
        for(i=0, j=HU_FONTSTART; i<HU_FONTSIZE; i++, j++)
        {
                ...
                if(j=='_') sprintf(lumpname, "DIG45");
                if(j=='(') sprintf(lumpname, "DIG91");
                if(j==')') sprintf(lumpname, "DIG93");

                hu_font[i] = lumpname[0] ?
                        W_CacheLumpName(lumpname, PU_STATIC) : NULL;
        }
        hu_fontloaded = true;
}
...
// each of which draws a different part.
        ...
        // the offset of percentage bars from the starting
        // text
#define GAP 40
